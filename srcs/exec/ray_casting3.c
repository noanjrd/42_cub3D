...
void calcul_distance_x(t_data *data, double *side_dist_x, int *step_x)
{
    ...
    {
        *step_x = -1;
        *side_dist_x = (data->player->x - data->player->map_x) * data->player->delta_x;
    }
    else
    {
        *step_x = 1;
        *side_dist_x = (data->player->map_x + 1.0 - data->player->x) * data->player->delta_x;
    }
}
...
void calcul_distance_y(t_data *data, double *side_dist_y, int *step_y)
{
    ...
    {
        *step_y = -1;
        *side_dist_y = (data->player->y - data->player->map_y) * data->player->delta_y;
    }
    else
    {
        *step_y = 1;
        *side_dist_y = (data->player->map_y + 1.0 - data->player->y) * data->player->delta_y;
    }
}